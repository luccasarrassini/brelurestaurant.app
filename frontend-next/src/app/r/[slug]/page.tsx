...
export default function PublicMenuPage() {
  ...
  useEffect(() => {
    ...
    async function loadRestaurant() {
      if (!slug) {
        if (active) { setState('not_found'); setMessage('Slug ausente na URL.') }
        return
      }

      if (!isValidSlug(slug)) {
        if (active) { setState('not_found'); setMessage('Slug invalido.') }
        return
      }
      ...
      if (!data) { setState('not_found'); setMessage(`Nenhum restaurante encontrado para o slug "${slug}".`); return }
  ...
  function addToCart(productId: string) {
    setCartItems((prev) => {
      ...
      if (!existing) return [...prev, { product_id: productId, quantity: 1 }]
