...
export default function MenuManager() {
  ...
  async function updateCategoryOrder(updated: Category[]) {
    ...
    await Promise.all(
      updated.map((category, index) =>
        updateCategory(category.id, { sort_order: index + 1 }),
      ),
    )
    ...
    setProducts((prev) =>
      prev.map((product) =>
        product.category_id === categoryId
          ? updated.find((item) => item.id === product.id) ?? product
          : product,
      ),
    )
    await Promise.all(
      updated.map((product, index) =>
        updateProduct(product.id, { sort_order: index + 1 }),
      ),
    )
    pushToast('Ordem atualizada')
  }
  ...
  function goToNewItem(categoryId?: string) {
    if (categoryId) {
      navigate(`/menu/item/new?categoryId=${categoryId}`)
      return
    }
    navigate('/menu/item/new')
  }
  ...
  return (
    <AdminLayout
      ...
      actions={
        <button className="button-primary" type="button" onClick={() => navigate('/menu/category/new')}>
          <Plus size={18} />
          Nova categoria
        </button>
      }
      ...
      <div className="mm-categories-list">
        {categories.map((category, catIndex) => {
          ...
          return (
            <div
              key={category.id}
              className={`mm-category-card animate-in ${dragOverId === category.id ? 'mm-drag-over' : ''}`}
              style={{ animationDelay: `${catIndex * 0.06}s` }}
            >
              {/* ── Category Header ── */}
              <div
                className="mm-category-header"
                draggable
                onDragStart={(e) => {
                ...
                onDrop={(e) => {
                  ...
                  handleCategoryDrag(sourceId, category.id)
                }}
              >
                <div className="mm-category-left">
                  <span className="mm-drag-handle" title="Arrastar para reordenar">
                  ...
                  <div className="mm-category-info" onClick={() => toggleCategory(category.id)} style={{ cursor: 'pointer' }}>
                    <div className="mm-category-title-row">
                      <h3>{category.name}</h3>
                      {category.model && (
                        <span className="mm-model-badge">{category.model}</span>
                      )}
                      {category.is_active === false && (
                        <span className="badge badge-cancelled">Pausada</span>
                      )}
                    </div>
                    <span className="mm-category-count">
                ...
                <div className="mm-category-actions">
                  <button
                    ...
                    type="button"
                    onClick={() => goToNewItem(category.id)}
                  ...
                  <button
                    ...
                    type="button"
                    onClick={() => navigate(`/menu/category/${category.id}/edit`)}
                  ...
                  <button
                    ...
                    title={category.is_active === false ? 'Ativar categoria' : 'Pausar categoria'}
                  >
                    {category.is_active === false ? <Play size={15} /> : <Pause size={15} />}
                  </button>
                  <button
                  ...
                  <button
                    className="mm-action-btn mm-chevron-btn"
                    type="button"
              ...
              {isOpen && (
                <div className="mm-category-body">
                  {categoryProducts.length === 0 ? (
                    <div className="mm-empty-category">
                    ...
                    <div className="mm-product-list">
                      {categoryProducts.map((product) => {
                        ...
                        return (
                          <div
                            key={product.id}
                            className={`mm-product-row ${product.is_out_of_stock ? 'mm-out-of-stock' : ''}`}
                            draggable
                            onDragStart={(e) => {
                            ...
                            onDrop={(e) => {
                              e.stopPropagation()
                              const sourceId = e.dataTransfer.getData('text/plain')
                              handleProductDrag(category.id, sourceId, product.id)
                            }}
                          >
                            <span className="mm-drag-handle" title="Arrastar para reordenar">
                            ...
                            <div className="mm-product-thumb">
                              {image ? (
                              ...
                              {product.is_out_of_stock && (
                                <span className="mm-oos-badge">Esgotado</span>
                              )}
                            </div>

                            <div className="mm-product-info">
                              <strong className="mm-product-name">{product.name}</strong>
                              <span className="mm-product-price">
                                {formatCents(product.price_cents)}
                              </span>
                            </div>

                            <div className="mm-product-status">
                              {product.is_active !== false && !product.is_out_of_stock && (
                                <span className="mm-status-dot mm-status-active" title="Disponível" />
                              )}
                              {product.is_active === false && (
                                <span className="badge badge-cancelled" style={{ fontSize: 11 }}>Pausado</span>
                              )}
                            </div>
                            ...
                            <div className="mm-product-actions">
                              <button
                                className="mm-action-btn"
                                type="button"
                                onClick={() => navigator.clipboard.writeText(product.id).then(() => pushToast('ID copiado'))}
                                title="Copiar ID"
                              >
                                <Link2 size={14} />
                              </button>
                              <button
                                className="mm-action-btn"
                                type="button"
                                onClick={() => navigate(`/menu/item/${product.id}/edit`)}
                                title="Editar item"
                              >
                                <Pencil size={14} />
                              </button>
                              <button
                                className={`mm-action-btn ${product.is_out_of_stock ? 'mm-action-active' : ''}`}
                                type="button"
                                onClick={() => toggleOutOfStock(product)}
                                title={product.is_out_of_stock ? 'Marcar disponível' : 'Marcar esgotado'}
                              >
                                <PackageX size={14} />
                              </button>
                              <button
